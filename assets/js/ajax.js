        ...
        function displayTableJSON() {
            ...
            if ($obj.result == 1) {
                ...
                for (var i = 0; i < data.length; i++) {
                    mid = mid + '<div id="test1" class="col s12"><div class="row width-80"><div class="col s12 m6 l3">' +
                        '<div class="card"><div class="card-image waves-effect waves-block waves-light"><div class="grey-out hide">' +
                        '<i class="mdi-av-not-interested"></i></div>' +
                        '<img class="activator" src="assets/images/' + data[i].image_name + '"></div><div class="card-content">' +
                        '<span class="card-title activator grey-text text-darken-4">' + data[i].meal_name + '<i class="mdi-navigation-more-vert right"></i></span><p><span class="currency">&#162;' + data[i].meal_price + '</span></p> </div><div class="card-reveal">' +
                        '<span class="card-title grey-text text-darken-4">' + data[i].meal_name + '<i class="mdi-navigation-close right"></i>' + '</span><p>' + data[i].meal_desc + '</p>';
                }
                end = '<a class="btn-floating btn-large waves-effect waves-light red right"><i class="mdi-action-favorite"></i></a></div></div></div></div></div></div>';
                var content = mid + end;
